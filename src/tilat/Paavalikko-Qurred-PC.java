...
public class Paavalikko extends Pelitila{
	...
	private ArrayList<Kirjaimet> kirjaimet;
	...
	/**
	 * Luodaan uusi Paavalikko-luokan ilmentymä, ja toteuttaa samalla metodin {@link #alusta()}
	 */
	public Paavalikko() {
		alusta();
	}
	...
	/**
	 * Metodi, joka alustaa luokan ilmentymälle tärkeät tiedot
	 */
	@Override
	public void alusta() {
		//Koitetaan ladata kaikki kuvat
		try{
			kirjaimet = new ArrayList<Kirjaimet>();
			for (int i = 0; i < pelinnimi.length; i++) {
				Kirjaimet tmp = new Kirjaimet(pelinnimi[i], 0.98, 4, 400 + (40 * i), 450);
				kirjaimet.add(tmp);
			}
	...
	/**
	 * Syötteen käsittely, Tulee muokkautumaan jos luokat {@link Hiiri} ja {@link Nappaimisto} toteutetaan
	 */
	@Override
