...
public class Peli extends Pelitila{
		...
		for(int i = 0; i < objektit.size(); i++){
			...
			for(int y = i + 1; y < objektit.size(); y++){
				...
				if(objektit.get(i).leikkaavat(objektit.get(y))){
					...
					/*
					objektit.get(i).asetaSijainti(objektit.get(i).annaX() +- objektit.get(i).annaLiikeX(),
							objektit.get(i).annaY() + objektit.get(i).annaLiikeY());
					
					objektit.get(y).asetaSijainti(objektit.get(y).annaX() +- objektit.get(y).annaLiikeX(),
							objektit.get(y).annaY() + objektit.get(y).annaLiikeY());*/
					}
				}	
			}
	...
	public void kasitteleSyote() {
		if(Nappaimisto.annaPainettu() == KeyEvent.VK_ESCAPE  && pelikesken){
			if(paussi){
				paussi = false;
			}else{
				paussi = true;
